<template>
  <el-card class="query-info-card" shadow="never">
    ...
    <el-descriptions :column="column" border>
      <!-- API端点信息 -->
      <el-descriptions-item v-if="apiEndpoint" label="API端点">
        <el-link 
          :href="apiEndpoint" 
          target="_blank" 
          type="primary"
      ...
      <!-- 查询描述 -->
      <el-descriptions-item v-if="description" label="查询描述">
        <span class="query-description">{{ description }}</span>
      </el-descriptions-item>
      
      <!-- 查询参数 -->
      <el-descriptions-item v-if="hasQueryParams" label="查询参数">
      ...
      <!-- 结果统计 -->
      <el-descriptions-item v-if="resultCount !== null" label="结果数量">
        <el-tag type="success" size="large">
          {{ resultCount }} {{ resultUnit || '条记录' }}
        </el-tag>
      </el-descriptions-item>
      
      <!-- 响应时间 -->
      <el-descriptions-item v-if="responseTime" label="响应时间">
        <el-tag type="info" size="small">{{ responseTime }}</el-tag>
      </el-descriptions-item>
      
      <!-- 查询时间 -->
      <el-descriptions-item v-if="queryTime" label="查询时间">
